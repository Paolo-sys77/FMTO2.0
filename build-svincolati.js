// Genera svincolati-csv.js dal CSV "db enore ... senza fmto CSV.csv"
// I giocatori vengono aggiunti alla sezione Svincolati con schede identiche alle altre.

const fs = require('fs');
const path = require('path');

const csvPath = path.join(process.env.USERPROFILE || '', 'Downloads', 'db enore 12-03-26 senza fmto CSV.csv');
const outPath = path.join(__dirname, 'svincolati-csv.js');
...
// Seconda colonna "Posizione" (indice 42) = pos_ment
...
const jsContent =
  '// Svincolati da CSV (db enore 12-03-26 senza fmto) — generato da build-svincolati.js\n' +
  'var SVINCOLATI_EXTRA = ' + JSON.stringify(players) + ';\n';

fs.writeFileSync(outPath, jsContent, 'utf8');
console.log('Scritto', outPath, 'con', players.length, 'giocatori svincolati.');
